    ...
    <div className="bg-background text-foreground font-unica">
      ...
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4">
          <SectionTitle title="LIDERANÇA LEGAL" subtitle="Os visionários que transformaram uma ideia em realidade e continuam moldando o futuro da tecnologia no Brasil." center />
          <div className="grid md:grid-cols-2 gap-12 max-w-4xl mx-auto">
            {leadership.map((leader, index) => <Card key={index} className="bg-card border-legal/30 shadow-xl hover:shadow-legal-cyan/30 transition-all duration-300 transform hover:-translate-y-2">
                <CardContent className="p-8 text-center">
                  <Avatar className="w-32 h-32 mx-auto mb-6">
                    <AvatarImage src={leader.avatar} alt={leader.name} className="object-cover w-full h-full" style={{
                  imageRendering: 'auto',
                  filter: 'none'
                }} />
                  ...
                  <div className="flex flex-wrap justify-center gap-2">
                    {leader.values.map((value, valueIndex) => <span key={valueIndex} className="px-3 py-1 bg-legal-cyan/10 text-legal-cyan text-xs font-bold rounded-full">
                        {value}
                      </span>)}
                  </div>
                </CardContent>
      ...
      <section className="py-16 md:py-24 bg-gray-50">
        <div className="container mx-auto px-4">
          <SectionTitle title="FRAMEWORK V.E.R.A.™" subtitle="Nossos valores que guiam cada decisão, cada projeto e cada conexão que fazemos." center />
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, index) => <Card key={index} className="bg-white border-legal/20 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      ...
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4">
          <SectionTitle title="NOSSO TIME LEGAL" subtitle="Cada pessoa do nosso time contribui para conectar pessoas, negócios e coisas em todo o Brasil." center />
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {teamMembers.map((member, index) => <Card key={index} className="bg-card border-legal/20 shadow-lg hover:shadow-legal-cyan/20 transition-all duration-300 transform hover:-translate-y-1">
                <CardContent className="p-6 text-center">
      ...
      <section className="py-16 md:py-24 bg-legal text-white">
        <div className="container mx-auto px-4 text-center">
          <SectionTitle title="SOMOS LEGAL EM NÚMEROS" subtitle="Dados que mostram nossa força e crescimento constante." center light />
          <div className="grid md:grid-cols-4 gap-8 max-w-4xl mx-auto">
            ...
            <div className="text-center">
              <div className="text-4xl md:text-5xl font-black text-legal-cyan mb-2">200K+</div>
